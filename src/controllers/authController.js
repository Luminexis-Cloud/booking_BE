...
class AuthController {
  ...
  async createAccountBasic(req, res, next) {
    ...
    try {
      ...
      console.log(`[${requestId}] createAccountBasic called`, { email });

      // Validation
      if (!name || !email) {
        console.warn(`[${requestId}] Missing name or email`);
      ...
      if (!password || !confirmPassword) {
        console.warn(`[${requestId}] Missing password fields`);
        return res.status(400).json({
          success: false,
      ...
      if (password !== confirmPassword) {
        console.warn(`[${requestId}] Password mismatch for ${email}`);
        return res.status(400).json({
          success: false,
      ...
      if (existing) {
        console.warn(`[${requestId}] Email already exists: ${email}`);
      ...
      console.error(`[${requestId}] createAccountBasic ERROR`, err);
